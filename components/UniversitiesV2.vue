<template>
  <div class="light-white-bg py-16">
    ...
    <v-container class="mt-9 mt-md-16">
      ...
      <div v-if="partnersCount < 1" class="mt-10 text-center">
        {{ $t('nothing_found') }}
      </div>
      <div v-else class="mt-10 partners">
        <div
          v-for="(item, index) in paginatedPartners"
          :key="index"
...
export default {
  ...
  computed: {
    paginatedPartners() {
      return [...this.partners].splice(0, this.pageSize)
    },
    partnersCount() {
  ...
  methods: {
    async fetchPartners(params = {}, isFirst = false) {
      params = {
        ...params,
        country_name: this.countryName,
      }
      if (!this.countryName) delete params.country_name

      await this.$axios
        .$get(`${this.$i18n.locale}/partners/`, { params })
        .then((res) => {
          if (isFirst) this.allPartners = res
          this.partners = res
    ...
    getInfo(item) {
      return item.partner_translations
        ? this.getByLanguage(item.partner_translations)
        : {}
    },
    goToPartner(item) {
